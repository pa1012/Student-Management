...
Account Login::checkAccount(ArrOfAccount Acc, ArrayOfCourse C) {
	int x = -1;
	Account wrongUsername,wrongPassword;
	wrongUsername.createAccount("0", "0", -1,"","","","",0,"","",  C);
	wrongPassword.createAccount("0", "0", -2,"","","","",0, "", "",  C);
...
int Login::handleEvent(const sf::Event &event, ArrOfAccount Acc, Account &A,ArrayOfCourse C) {
	if (state == "START") {
		string c = "";
