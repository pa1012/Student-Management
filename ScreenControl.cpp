...
string Screen::handleEvent(sf::Event event, string now, ArrOfAccount &Acc, string nowAdmin , string nowStudent, string nowLecturer) {
	
	if ((nowAdmin == "anywhere"  || nowStudent == "anywhere" || nowLecturer == "anywhere")&& now != "change password" && event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
